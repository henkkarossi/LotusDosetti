...
public class PillDispenserMotorScheduleFromListDemo {
	public static void main(String[] args) {
		LocalDateTime now = LocalDateTime.now();

		List<LocalDateTime> dosetti = new ArrayList<LocalDateTime>();

		// there are 14 slots, let's make a map with slot id and localdatetime as
		// timetotake for every ten seconds onward
		LocalDateTime newTimeToTake = now.plusSeconds(10);

		for (int i = 1; i < 15; i++) {
			dosetti.add(newTimeToTake);
			newTimeToTake = newTimeToTake.plusSeconds(10);
		}

		System.out.println(dosetti);
		...
		final ScheduledFuture<?> rmotorHandle = scheduler.schedule(rmotor, delay, SECONDS);
